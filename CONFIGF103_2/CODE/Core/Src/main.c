...
uint8_t newCard[5] = {0xff,0xff,0xff,0xff,0xff};
...
unsigned int TT_CUA = 0;//COI
unsigned int TT_IR = 0;//HONG NGOAI CUA
unsigned int TT_MODE = 0;//CUA DONG MO
...
char Str_QUAT3[200];
char Str_COI[200];
char Str_CUA[200];//COI CUA
char Str_IR[200];//HONG NGOAI
char Str_MODE[200];// DONG MO CUA
...
void SendDataJson(unsigned int ND , unsigned int DA ,
                  ...
                  unsigned int TT_CUA, unsigned int TT_IR, unsigned int TT_MODE);
//===================================================
// UART1 ESP8266
#define SizeBuf 200
...
static uint32_t buzzerON = 10;  // 10ms
static uint32_t buzzerOff = 300;  // 300ms
static uint32_t buzzerLoop = 0;  //
...
int main(void)
{
 ...
 // BuzzerSetLoop(5);
  ...
  EE_ReadVariable(31, &new_id); //ban dau se bang 1
...
void XuLyJson(char *DataJson)
{
  ...
  else
  {
    ...
    else if (cJSON_GetObjectItem(str_json, "OPENCLOSE"))
    {
      DieuKhienCua();
      SendDataJson(ND , DA ,
                   TT_DEN1 , TT_DEN2 ,
    ...
    else if (cJSON_GetObjectItem(str_json, "MODE"))
    {
      if (strstr(cJSON_GetObjectItem(str_json, "MODE")->valuestring, "0") != NULL)
      {
        TT_MODE = 0;

      }
      else if (strstr(cJSON_GetObjectItem(str_json, "MODE")->valuestring, "1") != NULL) //bien dang o dang string
      {
        TT_MODE = 1;
...
void SendDataJson(unsigned int ND , unsigned int DA ,
                  ...
                  unsigned int TT_QUAT1 , unsigned int TT_QUAT2 ,
                  unsigned int TT_QUAT3, unsigned int TT_COI ,
                  unsigned int TT_CUA, unsigned int TT_IR ,
                  unsigned int TT_MODE)
{
  ...
  sprintf(Str_CUA, "%d", TT_CUA);
  sprintf(Str_IR, "%d", TT_IR);
  sprintf(Str_MODE, "%d", TT_MODE);
  //{"ND":"123","DA":"456"}
...
void DieuKhienCua(void)
{
  // phai an vao 2 lan nen ko dieu khien nhu tren dc
  TT_CUA = 1;
  servo_start = true;

  ServoStart();
  lastServo = HAL_GetTick();
  printf("JSON da xu dc SERVO");
...
void ServoStart(void)
{
  htim2.Instance -> CCR2 = map(180, 0, 180, 600, 1800); //2200
}
void Start_Servo_RFID(void)
{
...
void Read_RFID_RC522(void)
{
  ...
  else if (HAL_GetTick() - last_rfid_start >= 15000 && rfid_entering == 0 && EVENT == MODE_XOA_THE)
  {
    EVENT = MODE_EXIT;
    BuzzerSetLoop(1);
  }
...
uint8_t MFRC522_CompareEEROM(uint8_t * CardID, uint16_t key_id) //1-5
{
  uint8_t i;
  uint8_t CompareID[5];
...
void Write_Key(uint8_t* id_card)
{
  ...
  {
    if (MFRC522_CompareEEROM(id_card, a) == MI_OK)
    {
      NoSameID = 0;//co the bi trung
      break;
    }
    else
    {
      NoSameID = 1;//khong co the bi trung
    }
  }
  ...
  {
    lcd_send_cmd (0x80 | 0x00);
    lcd_send_string("The da bi trung");
  }
  if (new_id > 5)
  {
    new_id = 1;
