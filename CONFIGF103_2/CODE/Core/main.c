...
unsigned int TT_BUZZER = 0;//COI
unsigned int TT_IR = 0;//HONG NGOAI CUA
unsigned int TT_SERVO = 0;//CUA DONG MO
...
char Str_QUAT3[200];
char Str_COI[200];
char Str_BUZZER[200];//COI CUA
char Str_IR[200];//HONG NGOAI
char Str_SERVO[200];// DONG MO CUA
...
void SendDataJson(unsigned int ND , unsigned int DA , 
									...
									unsigned int TT_BUZZER,unsigned int TT_IR,unsigned int TT_SERVO);
//===================================================
// UART1 ESP8266
#define SizeBuf 200
...
void XuLyJson(char *DataJson)
{
	...
	else
	{
		...
		else if(cJSON_GetObjectItem(str_json, "SERVO"))
		{
			 DieuKhienCua();
			 SendDataJson(ND , DA , 
									TT_DEN1 , TT_DEN2 , 
...
void SendDataJson(unsigned int ND , unsigned int DA , 
									...
									unsigned int TT_QUAT1 , unsigned int TT_QUAT2 , 
									unsigned int TT_QUAT3, unsigned int TT_COI ,
									unsigned int TT_BUZZER, unsigned int TT_IR ,
									unsigned int TT_SERVO)
{
...
//	}
	...
	for(uint8_t a = 1; a < 6; a++)
	{
		...
		{
			SameID = 0;//co the bi trung
			break;	 
		}
		else
		{
			SameID = 1;//khong co the bi trung	
		}
 	}
